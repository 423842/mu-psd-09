<template>
  <div id="app">
    ...
    <div v-if="result">
      ...
      <p v-if="result.transcription">
        <strong>テキスト:</strong> {{ result.transcription }}
      </p>
      <p v-else :style="{ color: 'gray', opacity: 0.5 }">
        音声が入力されていません
      </p>
...
export default {
  ...
  data() {
    return {
      ...
      tabAtRequestStart: null, // 解析開始時のタブを保持
      recordingTimeout: null, // 録音自動停止用のタイマー
  ...
  methods: {
    selectTab(tab) {
      ...
      // 解析中でない場合のみ、タブ切り替え時に結果とファイルをクリアする
      if (!this.loading) {
        this.result = null;
    ...
    async uploadFile() {
      ...
      this.loading = true;
      this.result = null;
      this.tabAtRequestStart = this.selectedTab; // 解析開始時のタブを記録
      ...
      try {
        ...
        // 結果をセットする前に、タブが変更されていないか確認
        if (this.selectedTab === this.tabAtRequestStart) {
